...
def powerset(s):
    ...
    """
    Returns the power set of a set viewed as a list
...
def CE(densityMatrix: np.matrix, s: list):

    """
    Returns the concentratable entanglement function described by a qubit subset s, when evaluated on a density matrix.

    Parameters:
    densityMatrix (np.matrix): The density matrix of the system.
    s (list): The qubit subset.

    Returns:
    float: The concentratable entanglement function.
    """

    N = int(np.log2(densityMatrix.shape[0]))
    ...
    for subset in PS:
        subset = list(subset)
        rhoSubset = partialTrace(subset, densityMatrix)
        puritySubset = abs(np.trace(rhoSubset@rhoSubset))
        value -= 1/(2**len(s))*puritySubset
